...
export interface RelevanceCtx {
  seedISODate: string;  // "YYYY-MM-DD" – SSR és kliens ugyanaz
  hourLocal?: number;   // 0..23 – ha nincs, 12-t feltételezünk
}

/* ---------- Helpers ---------- */
function parseList<T extends string>(v?: string | T[]): T[] {
  if (!v) return [];
  return Array.isArray(v) ? v : v.split(",").map(s => s.trim()).filter(Boolean) as T[];
}
...
function nearSeason(a: Exclude<Season,"egész évben">, b: Exclude<Season,"egész évben">) {
  ...
  const d = Math.min((4 + j - i) % 4, (4 + i - j) % 4); // 0..3
  return d;
}
...
/* „étkezés_után” – akkor preferált, ha 2 órán belül volt tipikus étkezés */
...
function daypartScore(tea: Tea, ctx: RelevanceCtx): number {
  ...
  // BÓNUSZOK a speciálisakra (kontextus szerint):
  let special = 0;
  ...
  if (rec.includes("bármikor")) return Math.max(15, special); // kis, de nem nulla plusz

  // Normál naprészek: pontos 40, szomszéd 20
...
function servingScore(tea: Tea, ctx: RelevanceCtx): number {
  ...
  if (s === "nyár") {
    ...
    // tavasz/ősz: kis előny, ha bármelyik explicit támogatott
...
function caffeineScore(tea: Tea, ctx: RelevanceCtx): number {
  ...
  const h = ctx.hourLocal ?? 12;
  if (h <= 11) return Math.min(30, Math.round(c * 0.3));   // reggel enyhe plusz
  if (h >= 17) return -Math.min(100, Math.round(c * 1.0)); // este erős mínusz
  return 0; // napközben semleges
}

/* ---------- Fő függvény + tie-break ---------- */
export function computeRelevance(tea: Tea, ctx: RelevanceCtx): number {
  let s = 0;
  s += seasonScore(tea, ctx);   // max ~100
  s += daypartScore(tea, ctx);  // max ~40 (+specialok)
  s += servingScore(tea, ctx);  // ±25
  s += caffeineScore(tea, ctx); // -100..+30
  return s;
}
